...
def load_dataset(dataset_ref: str, conn: Optional[duckdb.DuckDBPyConnection] = None) -> pd.DataFrame:
    """
    ...
    if os.path.exists(dataset_ref):
        ...
        try:
            if ext == '.csv':
                ...
                return pd.read_excel(dataset_ref)
            elif ext == '.json':
    ...
    try:
        ...
        df = conn.execute(f'SELECT * FROM "{dataset_ref}"').df()
        return df
    except Exception as db_err:
        # If it's a file path and we tried view-based selection, try DuckDB CSV reader directly
        if os.path.exists(dataset_ref):
            try:
                df = conn.execute(f"SELECT * FROM read_csv_auto('{dataset_ref}')").df()
                return df
            except Exception:
